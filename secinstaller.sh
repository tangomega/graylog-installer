...
configure_https_graylog() {
  section "Configuring HTTPS for Graylog Web UI"
  type_echo "[HACKER] Enabling TLS/HTTPS for Graylog Web Interface..."

  sudo mkdir -p /etc/graylog/server/certs
  local cert_file="/etc/graylog/server/certs/graylog.crt"
  local key_file="/etc/graylog/server/certs/graylog.key"
  ...
  # Secure permissions
  sudo chown graylog:graylog "$cert_file" "$key_file"
  sudo chmod 600 "$key_file"

  # Update Graylog server.conf
  sudo sed -i '/^http_enable_tls/c\http_enable_tls = true' /etc/graylog/server/server.conf
  sudo sed -i "/^http_tls_cert_file/c\http_tls_cert_file = $cert_file" /etc/graylog/server/server.conf
  sudo sed -i "/^http_tls_key_file/c\http_tls_key_file = $key_file" /etc/graylog/server/server.conf

  log "Graylog HTTPS enabled with self-signed cert."

  sudo systemctl restart graylog-server.service >/dev/null 2>&1 &
  spinner_with_runner $! "Restarting Graylog server with TLS..."
  log "Graylog now available at: https://<server-ip>:9000"
}
