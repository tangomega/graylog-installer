...
set_mongo_bindipall() {
  ...
  backup_file "$conf"
  # Check if correctly configured (net: { port: 27017, bindIpAll: true } present, no bindIp or nnet)
  if grep -A2 "^[[:space:]]*net:" "$conf" | grep -q "port: 27017" && grep -A2 "^[[:space:]]*net:" "$conf" | grep -q "bindIpAll: true" && ! grep -q "^[[:space:]]*bindIp:" "$conf" && ! grep -q "^[[:space:]]*nnet:" "$conf"; then
    echo "MongoDB net: { port: 27017, bindIpAll: true } already set correctly."
    return
  fi
  # Remove any existing net:, nnet:, bindIp:, and standalone port: entries
  sed -i '/^[[:space:]]*\(net:\|nnet:\)/,/^[[:space:]]*[a-zA-Z#]/d' "$conf"
  ...
  echo "Set MongoDB net: { port: 27017, bindIpAll: true }, removed bindIp, nnet:, and duplicate net: sections (cite: https://www.mongodb.org/docs/manual/tutorial/install-mongodb-on-ubuntu/)."
}
...
main() {
  ...
  if ! start_and_wait "mongod"; then
    handle_failure "MongoDB" "/etc/mongod.conf" "journalctl -u mongod -n 200" "sudo systemctl restart mongod" "sed -i '/^[[:space:]]*\\(net:\\|nnet:\\)/,/^[[:space:]]*[a-zA-Z#]/d; /^[[:space:]]*bindIp:/d; /^[[:space:]]*port:/d; /^[[:space:]]*# network interfaces/a\\net:\\n  port: 27017\\n  bindIpAll: true' /etc/mongod.conf"
  fi
